...
class AdminRelatoriosController extends Controller
{
    ...
    public function indexAlunosPorInst()
    {

        $instituicoes = Escola::lists('nome', 'id');

        return view('admin.relatorios.index_alunos_por_inst', compact('instituicoes'));
    }


    public function relatorioAlunosPorInstituicao(Request $request)
    {


        if ($request['instituicao'] != null):
            $relatorio = Aluno::where('escola_id', $request['instituicao'])->get();

            $instituicao = Escola::find($request['instituicao']);

            $config = $this->Configuracoes;
            $pdf = \App::make('dompdf.wrapper');
            $pdf->loadView('admin.relatorios.aluno_por_instituicao', compact('relatorio', 'config', 'instituicao'));
            return $pdf->stream();

        else:

            return back()->withErrors('Selecione uma instituição');
